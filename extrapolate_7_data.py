...
print(x_t2.shape)
zt = torch.from_numpy(gt[:,idx[:,0]]).unsqueeze(2)
zt1 = torch.from_numpy(gt[:,idx[:,1]]).unsqueeze(2)
...
np.save(f'extrapolation/reconstruction_7_data.npy', x_hats.detach().cpu().numpy())
# 1-step rollout
