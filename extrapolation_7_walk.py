...
#latent = np.load(f'latent/latent_l2best.npy')
...
x_t2 = torch.from_numpy(gt[:, idx[:,2:]]).cpu()
print(x_t2.shape) # 11, 397, 3, 1699, 3
zt = torch.from_numpy(latent[:,idx[:,0]]).unsqueeze(2) # 11, 397, 1, 2, 3
zt1 = torch.from_numpy(latent[:,idx[:,1]]).unsqueeze(2)
s = torch.arange(1,6).view(1, 1, 5, 1, 1)
        ...
        for i, z_traj in enumerate(zhats):
            for j, z_window in enumerate(z_traj):
                for k, z in enumerate(z_window):
                    ...
                    loss, relative_error, relative_error_s = test(x_hats[i,j,k], x_t2[i,j,k])
...
np.save(f'extrapolation/reconstruction_7_triplet.npy', x_hats.detach().cpu().numpy())
#np.save(f'extrapolation/reconstruction_2d.npy', x_hats.detach().cpu().numpy())
# 1-step rollout
