...
print(x_t1.shape)
start = torch.from_numpy(gt[:,idx[:,0]]).unsqueeze(2)
end = torch.from_numpy(gt[:,idx[:,-1]]).unsqueeze(2)
...
np.save(f'interpolation/reconstruction_5_data.npy', x_hats.detach().cpu().numpy())
# 1-step rollout
