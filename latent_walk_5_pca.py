...
x_t1 = torch.from_numpy(gt[:, idx[:,1:4]]).cpu() # 11, 399, 1699, 3
print(x_t1.shape)
start = torch.from_numpy(latent[:,idx[:,0]]).unsqueeze(2)
end = torch.from_numpy(latent[:,idx[:,2]]).unsqueeze(2)
...
print(z_hat_t1[...,0].shape) # 11, 399, 2
zx = reducer_x.inverse_transform(z_hat_t1[...,0])
print(zx.shape)
zx = zx.reshape(-1, 1699)
zx = sc_x.inverse_transform(zx)
zx = zx.reshape(11, 397, 1699, 1)

zy = reducer_y.inverse_transform(z_hat_t1[...,1])
zy = zy.reshape(-1, 1699)
zy = sc_y.inverse_transform(zy)
zy = zy.reshape(11, 399, 1699, 1)

zp = reducer_p.inverse_transform(z_hat_t1[...,2])
...
with torch.no_grad():
    with autocast(enabled=trainer.C.amp):
        for i, x_traj in enumerate(x_hats):
            for j, z_window in enumerate(x_traj):
...
np.save(f'interpolation/reconstruction_5_pca_per_attr.npy', x_hats.detach().cpu().numpy())
# 1-step rollout
