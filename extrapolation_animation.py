...
# ani = FuncAnimation(fig, update, frames=399, interval=200)
...
import json
with open('extra_err_ot_90.json', 'r') as f:
    data = json.load(f)
...
attr_short = ['X', 'Y', 'P']
plot_titles = ['Ground Truth ({}GT)', 'Extrapolated {} ({}Ext)', '{}GT - {}Ext']
...
sc_extra  = [None for _ in range(3)]
...
for i, this_ax in enumerate([xlax, ylax, plax]):
    ...
    sc_extra[i] = this_ax.scatter([], [], alpha=1, c='purple', s=25, label='Extrapolated')
...
for i, this_ax in enumerate([xeax, yeax, peax]):
    ...
    this_ax.set_ylim(min(loss[i]) * 1.1, max(loss[i]) * 1.1)
...
def update(frame):
    ...
    for i in range(3):
        ...
        # Compute extrapolated point (purple)
        extra = 2 * latent[0, frame+1, :, i] - latent[0, frame, :, i]
        sc_extra[i].set_offsets(extra[:2])

        # Plot line from t0 extrapolated to actual t2
        line_dashed[i].set_data(
            [latent[0, frame, 0, i], extra[0]],
            [latent[0, frame, 1, i], extra[1]]
        )
        line_residual[i].set_data(
            [extra[0], latent[0, frame+2, 0, i]],
            [extra[1], latent[0, frame+2, 1, i]]
        )
...
ani.save('test_extrapolate_optimized.gif', writer='pillow')
plt.show()