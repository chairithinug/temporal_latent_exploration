...
print(x_t1.shape) # 11, 397, 3, 1699, 3
start = torch.from_numpy(latent[:,idx[:,0]]).unsqueeze(2) # 11, 397, 2, 3
end = torch.from_numpy(latent[:,idx[:,-1]]).unsqueeze(2)

x_hats = torch.zeros((11, 397, 3, 1699, 3), device='cpu')
alphas = torch.linspace(0, 1, steps=5)[1:-1]
alphas = alphas.view(1, 1, 3, 1, 1)
print(alphas.shape)
print(start.shape)
...
name = 'inter5_err_ot_90'
#name = 'inter5_err_ot'
'''
with torch.no_grad():
    with autocast(enabled=trainer.C.amp):
        for i, z_traj in enumerate(zhats): # 11
            for j, z_window in enumerate(z_traj): # 397
...
np.save(f'interpolation/reconstruction_5_triplet.npy', x_hats.detach().cpu().numpy())
'''
# 1-step rollout

x_hats = torch.from_numpy(np.load(f'interpolation/reconstruction_5_triplet.npy')).cpu()
...
with torch.no_grad():
    with autocast(enabled=trainer.C.amp):
        for i in range(n): # timestep
            ...
            relative_error_total = 0
            relative_error_s_total = []
            for j in range(len(x_hats) if name =='inter5_err_ot' else 1): # traj
                for k in range(3):
